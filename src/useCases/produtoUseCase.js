...
class ProdutoUseCase {
  static async createProduto(data) {
    ...
    const { nome, qtd_estoque, categoria, preco, status, ultima_atualizacao } = data;
    ...
    const produto = new Produto(produtoId, nome, qtd_estoque, categoria, preco, status, ultima_atualizacao);
  ...
  static async updateProduto(id, data) {
    ...
    return new Produto(id, data.nome, data.qtd_estoque, data.categoria, data.preco, data.status, data.ultima_atualizacao);
  }
